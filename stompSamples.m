...
for m = 1 : nJoints
    ...
    [rows, ~] = size(gau_dis);
    zero_dis = zeros(rows,1);
    dis = cat(2, zero_dis, gau_dis, zero_dis);% horizontally append the 2 zreo distribution array and a gaussian distribution array for q0 and qT
    em{1,m}= dis;
end
